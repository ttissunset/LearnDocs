...
- **`loader` 本身是一个函数，==接受原文件作为参数，返回转换的结果==** 
  - `babel-loader`：将 `ES6+` 转换为低版本
  - `css-loader`：加载和解析 `.css` 文件
  - `style-loader`：将样式通过 `<style>` 标签插入到 `head` 中，确保样式生效
  - `file-loader`：进行图片、字体等文件的打包
  - `thread-loader`：多进程打包 `Js` 和 `CSS`

- `webpack` 遵循**单一职责**，一个 `loader` 只负责处理一件事
- 多个 `loader` 的执行顺序是**从右到左**的
...
- `Plugin` 是用来**扩展 `Webpack` 功能**的如：**代码压缩**、**代码分离**
  ...
  - `mini-css-extract-plugin`、`html-webpack-plugin`：代码分离
  - `html-webpack-plugin`：创建 `html` 文件引入打包输出的 `js`文件
...
- `Bable` 用于**将 ES6+ 的代码转换为低版本代码**，用于**处理浏览器兼容问题**
